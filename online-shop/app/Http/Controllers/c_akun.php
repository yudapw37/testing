...
class c_akun extends Controller
{
        ...
        $urlImage = 'https://admin.aqwam.com/';
            ...
            if ($request->session()->has('idUser')){
                if($request->session()->get('token')=='4qw4m5t0r3'){
                    ...
                    $user = mod_customer::select('id as idUser', 'username','userId', 'nama','email', 'alamat', 'diskon', 'telephone', 'jenis_reseller')->where('username', '=', $username)->first();
                    // dd($user);
                    $userId =$user->userId;
                    $alamat =$user->alamat;
                    $diskon = $user->diskon;
                    ...
                    'allcategories'=>$allcategories,  'countCart'=>$countCart, 'nama'=>$nama,  'username'=>$username,'userid'=>$userId,'email'=>$email,'alamat'=>$alamat, 'diskon'=>$diskon, 'telephone'=>$telephone, 'jenis_reseller'=>$jenis_reseller, 'promoviewhome'=>$promoviewhome]);
                }else{
    ...
    public function editakun(Request $request){
        if ($request->session()->has('idUser')){  
            $request->validate([
                'usernameedt'=>'required|max:99|min:1',
            ...
            mod_customer ::where('id', '=', $idUser)                             
                                ->where('username', '=', $username)
                                ->update(
                                    ['userId' => $request->usernameedt,'nama' => $request->namaedt, 'telephone'=>$request->hpedt, 
    ...
    public function editreseller(Request $request){
        ...
        $urlImage = 'https://admin.aqwam.com/';
            ...
            if($request->session()->get('token')=='4qw4m5t0r3'){
                ...
                'footerphone2'=>$footerphone2, 'footeraddrs2'=>$footeraddrs2, 'pathPublic'=>$pathPublic, 
                'getBaseUrl'=>$getBaseUrl,'urlImage'=>$urlImage, 'categories'=>$categories,'countcategories'=>$countcategories, 
                'allcategories'=>$allcategories,  'countCart'=>$countCart, 'nama'=>$nama,  'username'=>$username,'email'=>$email,'alamat'=>$alamat, 'diskon'=>$diskon, 'telephone'=>$telephone, 'jenis_reseller'=>$jenis_reseller, 'promoviewhome'=>$promoviewhome]);
            }else{
            return redirect('/')->with('statusLogin', 'Login Dahulu!');
            }
