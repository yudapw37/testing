...
class c_invoice extends Controller
{
    public function index(Request $request){
        ...
        if ($request->session()->has('idUser')){
            if($request->session()->get('token')=='4qw4m5t0r3'){
                ...
                $invoice = mod_invoice::select('id as idInvoice','idOrder','total_harga','biaya_expedisi','totalDiskon','diskonKodeUnik','nama_penerima','created_at')->where('code_customer', '=', $idUser )->orderBy('created_at', 'DESC')->skip($page)->take(5)->get(); 
            //     if($idbuku){
            ...
            if ($request->session()->has('idUser')){
                if($request->session()->get('token')=='4qw4m5t0r3'){
                    ...
                    if($buku){  
                        ...
                        'totaldiskon'=>$totaldiskon, 'totaldiskonrupiah'=>$totaldiskonrupiah, 'totalhargarupiah'=>$totalhargarupiah]);
                }
               }else{
            ...
            if ($request->session()->has('idUser')){
                if($request->session()->get('token')=='4qw4m5t0r3'){
                    ...
                    return view('invprint', ['pathhf'=>$pathhf, 'pathcs'=>$pathcs, 'getBaseUrlEnv'=>$getBaseUrlEnv, 
                        'headerphone'=>$headerphone, 'footerphone1'=>$footerphone1, 'footeraddrs1'=>$footeraddrs1, 'footerphone2'=>$footerphone2, 'footeraddrs2'=>$footeraddrs2,
                        'pathPublic'=>$pathPublic, 'getBaseUrl'=>$getBaseUrl, 'categories'=>$categories,'countcategories'=>$countcategories, 'allcategories'=>$allcategories,
                        'countCart'=>$countCart, 'nama'=>$nama, 'cart'=>$cart,'invdetail'=>$invdetail,'inv'=>$inv, 'namaadmin'=>$admin->nama]);
               }else{
        ...
        // dd($id);
            ...
            if ($request->session()->has('idUser')){
                ...
                return view('confirm', ['pathhf'=>$pathhf, 'pathcs'=>$pathcs, 'getBaseUrlEnv'=>$getBaseUrlEnv, 
                        ...
                        'id'=>$id, 'totalbayar'=>$totalbayar, 'hasil_rupiah'=>$hasil_rupiah]);
               }else{
    ...
    public function url(){
        return 'storeaqwam.cemaraitsalatiga.com/uploads/';
    }
}
